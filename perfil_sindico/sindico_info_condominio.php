...
$seguindo = $con->verificarSeguindo($_SESSION['UsuarioID'],$id_condominio);
							...
							<div class="vagas-opcao-cont">
									...
									<div class="banner-perfil">
										<div class="linha">
											...
											<div class="nome-perfil">
												<h2><?=$dados_cond['nome_cond']?></h2>
												<hr>
												<h3><?=$dados_cond['tipo_cond']?></h3>
											</div>
										</div>
									</div>
								</div>
								<div class="texto texto-infos-vagas" style="background-color: #fff">
									<div style="width: 96%; padding: 5px 2%">
										<?php if($seguindo['seguindo'] == ""){ ?>
										<form style="padding-bottom: 10px" method="post" action="../paginasPHP/seguindo_cond.php">
											<input type="hidden" name="id_usuario" value="<?= $_SESSION['UsuarioID'] ?>">
											<input type="hidden" name="id_condominio" value="<?=$dados_cond['id']?>">
											<input type="hidden" name="seguindo" value="1">
											<button type="submit" class="submit">Seguir o condomínio</button>
										</form>
									<?php }else if($seguindo['seguindo'] == 0){ ?>
										<form style="padding-bottom: 10px" method="post" action="../paginasPHP/seguindo_cond.php">
											<input type="hidden" name="id_usuario" value="<?= $_SESSION['UsuarioID'] ?>">
											<input type="hidden" name="id_condominio" value="<?=$dados_cond['id']?>">
											<input type="hidden" name="seguindo" value="1">
									...
									<?php }else{ ?>
										<form style="padding-bottom: 10px" method="post" action="../paginasPHP/seguindo_cond.php">
											<input type="hidden" name="id_usuario" value="<?= $_SESSION['UsuarioID'] ?>">
											<input type="hidden" name="id_condominio" value="<?=$dados_cond['id']?>">
											<input type="hidden" name="seguindo" value="0">
											<button type="submit" class="reset">Quero deixar de seguir o condomínio</button>
										</form>
									<?php } ?>

...
$con = new ConexaoAdm();
	...
	if(!empty($_POST['nome'])  && !empty($_POST['senha']) && !empty($_POST['email'])){
       ...
       //endereco
		...
		$sql = "UPDATE usuarios SET nome='$nome', email='$email', telefone='$tel_cel', senha='$senha', cpf='$cpf', telefone_fixo='$telefone_fix', data_aniversario='$data_aniversario', sexo='$sexo', apresentacao='$apresentacao', idiomas='$idiomas', link_video='$link_video', cep='$cep', pais='$pais', estado='$estado', cidade='$cidade', endereco='$endereco', complemento='$complemento', numero_casa='$numero_casa',nivel='$nivel',liberado='$liberado' WHERE id='$id_usuario'";

        $con->AtualizarSindico($sql,$id_usuario, $nome_empresa, $cargo,$inicio,$fim, $situacao, $array_curso);
        header("Location: ../adm_solicitante_perfil.php?idh=$id"); exit;
	}else{
		die("erro");
	}
?>